...
export function ServicesSection() {
  return (
    <section id="servicos" className="border-t border-amber-300/30 bg-[#ededed]">
      <div className="mx-auto max-w-7xl px-4 py-18 sm:py-24">
        <Reveal>
          <div className="mb-10 sm:mb-14">
            <p className="text-xs tracking-[0.22em] text-amber-700">NOSSOS SERVICOS</p>
            <h2 className="mt-3 font-display text-3xl text-zinc-900 sm:text-4xl">Música ao vivo para momentos eternos</h2>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {SERVICES.map((item, i) => (
            <Reveal key={item.title} delayMs={i * 120}>
              <article className="group rounded-2xl border border-amber-300/35 bg-white p-7 shadow-[0_14px_40px_rgba(0,0,0,0.08)] transition hover:-translate-y-1 hover:border-amber-500/60">
                <h3 className="font-display text-2xl text-amber-700">{item.title}</h3>
